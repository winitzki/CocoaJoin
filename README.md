...
The reaction is written as a pure function that takes each input molecule's value as an argument. For instance, consider the reaction
	consume a(x), b(y) => let x = compute_whatever(x,y); inject c(x),a(0),a(1),a(2) // whatever
This reaction takes `x` and `y` as arguments and computes a function.
...
The user can certainly add new reactions that consume other molecules and _inject_ `inc` or `get`. However, the user cannot define a new reaction that, say, consumes `inc` or `get`. If the user of the `define_counter` tries to define a new reactions that consumes `inc`, say

consume inc() => print "gotcha"

- the result will be that a _new_ molecule name `inc` is defined, with this new reaction. This is so because any join definition always _defines_ the input molecule names as new values in the local scope.
...
DinPhil5 is a test project that shows a simple solution of the "dining philosophers" problem in a barebones UI. The logic is implemented in "DiningPhilosophicalLogic.m" which uses a reaction on the UI thread. Note that the join definition is asynchronous and all other reactions are also asynchronous.

Available macros
----------------

Make a local join definition:
cjDef(...)   -- a join definition with decision code on a background thread
cjDefUI(...) -- a join definition with decision code on the UI thread

Define a new input molecule name:

cjAsync(name, t) -- slow molecule with value of type t
cjSync(tout, name, tin) -- fast molecule with value of type tin, returning value of type tout
...
Similar macros cjReact3, cjReact3UI, cjReact4, and cjReact4UI are available.
...
cjReply(name, val) -- reply with value `val` to a fast molecule named `name`
